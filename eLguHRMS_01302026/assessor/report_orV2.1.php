...
// Output payor name with larger font only
$html = '<span style="font-size:12pt;">' . $payor . '</span>';
$pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, 'L', true);


// // Previous Year
// Output date with larger font only
// $pdf->SetXY(215, 49);
...
// Output total amount with larger font only
...
foreach ($data as $row) {
    ...
    $pdf->Cell(10, 0, strtoupper($row['classification']), 0, 0, 'L'); // Class
    $pdf->Cell(25, 0, number_format($row['assessed_value'], 2), 0, 0, 'R'); // Assessed
    $pdf->Cell(22, 0, number_format($row['basic_tax'], 2), 0, 0, 'R'); // Basic
    $pdf->Cell(23, 0, number_format($row['sef_tax'], 2), 0, 0, 'R'); // Sef
    $pdf->Cell(22, 0, number_format($row['discount'], 2), 0, 0, 'R'); // Penalty
