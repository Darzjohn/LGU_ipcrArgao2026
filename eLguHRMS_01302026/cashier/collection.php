...
if ($search !== '') {
    ...
    $where = "WHERE (owner_name LIKE '%$esc%' OR td_no LIKE '%$esc%' OR lot_no LIKE '%$esc%' OR rptsp_no LIKE '%$esc%' OR or_no LIKE '%$esc%' OR processed_by LIKE '%$esc%')";
}
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <form method="get" class="row g-2">
                <div class="col-md-4">
                    <input type="text" name="search" class="form-control" placeholder="Search by Owner, TD No, Lot No, OR No, Cashier/Collecting Officer" value="<?=htmlspecialchars($search)?>">
                </div>
                ...
                <table class="table table-bordered table-hover align-middle">
                    <thead class="table-dark text-center">
                        <tr>
                            ...
                            <th>Processed By</th>
                            <th>Status</th>
                            <!-- ✅ Add hidden fields to include previous payment info -->
                            <th style="display:none;">Prev OR No</th>
                            <th style="display:none;">Prev Date Paid</th>
                            <th style="display:none;">Prev Year</th>
                        </tr>
                    </thead>
                    <tbody>
                                ...
                                <tr>
                                    <td class="text-center">
                                        <input type="checkbox" 
                                               ...
                                               class="form-check-input record-check"
                                               data-prev-or="<?= htmlspecialchars($row['previous_or_no'] ?? '') ?>"
                                               data-prev-date="<?= htmlspecialchars($row['previous_date_paid'] ?? '') ?>"
                                               data-prev-year="<?= htmlspecialchars($row['previous_year'] ?? '') ?>">
                                    </td>
                                    <td><?= $row['id'] ?></td>
                                    ...
                                    <td style="display:none;"><?= htmlspecialchars($row['previous_or_no'] ?? '') ?></td>
                                    <td style="display:none;"><?= htmlspecialchars($row['previous_date_paid'] ?? '') ?></td>
                                    <td style="display:none;"><?= htmlspecialchars($row['previous_year'] ?? '') ?></td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
...
document.getElementById("btnReprint").addEventListener("click", function() {
    ...
    selected.forEach(ch => {
        ...
        // ✅ Add hidden previous info for each selected record
        ['prev_or', 'prev_date', 'prev_year'].forEach(attr => {
            const hidden = document.createElement('input');
            hidden.type = 'hidden';
            hidden.name = attr + '[]';
            hidden.value = ch.dataset[attr.replace('prev_', 'prev-')];
