...
<script>
// ================= Duplicate CTC Check =================
async function checkDuplicateCTC(input) {
    const val = input.value.trim();
    if (!val) return { duplicate: false };
    try {
        const res = await fetch('ctccorp_list.php?check_ctccorp_no=' + encodeURIComponent(val));
...
// On blur, check duplicates
document.querySelectorAll('[name="ctccorp_no"]').forEach(input => {
    input.addEventListener('blur', () => checkDuplicateCTC(input));
});
