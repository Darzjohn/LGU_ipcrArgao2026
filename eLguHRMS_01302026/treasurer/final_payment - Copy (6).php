...
// ✅ Validate total_cash_amount and check_number
$total_cash_amount = !empty($_POST['total_cash_amount']) ? floatval($_POST['total_cash_amount']) : 0.0;
...
// ✅ Fetch user's "name" from users table based on session user_id
$collecting_officer = 'System User';
if (!empty($_SESSION['user_id'])) {
    $uid = intval($_SESSION['user_id']);
    $user_stmt = $mysqli->prepare("SELECT name FROM users WHERE id = ? LIMIT 1");
    $user_stmt->bind_param('i', $uid);
    $user_stmt->execute();
    $user_stmt->bind_result($db_name);
    if ($user_stmt->fetch() && !empty($db_name)) {
        $collecting_officer = $db_name;
    }
    $user_stmt->close();
}
...
try {
    ...
    $stmt = $mysqli->prepare("
        INSERT INTO collections (
            ...
            check_date, check_amount, total_cash_amount, check_number
        ) VALUES (
            ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
        )
    ");
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $stmt->bind_param(
            "ssssiisssssssddddddddsssssiddddssds",
            $or_no,
            $previous_or_no,
            $previous_date_paid,
            $previous_year,
