...
if (isset($_POST['add_ctccorp'])) {
    q("INSERT INTO ctc_corporation (
        ...
        business_address, kind_of_org, incorporation_address, date_reg,
        ...
        $_POST['company_fullname'], $_POST['business_address'], $_POST['kind_of_org'],
...
if (!empty($_GET['search'])) {
    $where .= " AND (company_fullname LIKE ? OR ctccorp_no LIKE ?)";
    $search = "%" . $_GET['search'] . "%";
...
<div class="container-fluid mt-4">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h4><i class="bi bi-building"></i> Corporation CTC List</h4>
    <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addCTCCorpModal">➕ New Corp CTC</button>
  </div>

  <form class="row mb-3" method="get">
    <div class="col-md-4">
      <input type="text" name="search" class="form-control" placeholder="Search by CTC No. or Company"
      ...
      <table class="table table-bordered table-hover align-middle text-center">
        <thead class="table-dark">
          <tr>
            ...
            <th>RPT Tax</th><th>GR Tax</th><th>Basic Tax</th><th>Total Due</th><th>Date Issued</th><th>Treasurer</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
...
<script>
// ✅ Auto-compute Surcharge & Total Due for both Add and Edit modals
document.addEventListener('input', function(e) {
  ...
  if (t.closest('.modal')) {
    ...
    // ✅ Match CTC Individual surcharge rule:
    // If issued after February, 2% monthly increment starting March up to 24% max
    const dateField = modal.querySelector('[name="date_issued"]');
    let surchargeRate = 0;
    if (dateField && dateField.value) {
      const d = new Date(dateField.value);
      const month = d.getMonth() + 1;
      if (month > 2) surchargeRate = Math.min(((month - 2) * 2), 24);
    }
