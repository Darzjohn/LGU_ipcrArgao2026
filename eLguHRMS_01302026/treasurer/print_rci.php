...
/* ------------------------------
   GET SELECTED IDS FROM POST (checkboxes)
------------------------------ */
$ids_array = $_POST['selected'] ?? [];
if (empty($ids_array) || !is_array($ids_array)) {
    die("<script>alert('No checks selected'); window.close();</script>");
}
...
<tr>
    <td>BANK Name/Account No: <?= !empty($chunk[0]['account_name']) ? htmlspecialchars($chunk[0]['account_name'] . '/' . $chunk[0]['account_number']) : 'UNKNOWN ACCOUNT' ?></td>
