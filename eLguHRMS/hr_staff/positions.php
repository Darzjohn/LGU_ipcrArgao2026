...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_position'])) {
    ...
    if ($name !== '') {
        $stmt = $mysqli->prepare("INSERT INTO positions (name) VALUES (?)");
        $stmt->bind_param("s", $name);
        $stmt->execute();
        $stmt->close();

        $messages[] = ['type' => 'success', 'text' => 'Position added successfully!'];
    } else {
        $messages[] = ['type' => 'danger', 'text' => 'Position name is required!'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_position'])) {
    ...
    if ($id > 0 && $name !== '') {
        $stmt = $mysqli->prepare("UPDATE positions SET name=? WHERE id=?");
        $stmt->bind_param("si", $name, $id);
        $stmt->execute();
        $stmt->close();

        $messages[] = ['type' => 'success', 'text' => 'Position updated successfully!'];
    } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($id > 0) {
        $stmt = $mysqli->prepare("DELETE FROM positions WHERE id=?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->close();

        $messages[] = ['type' => 'success', 'text' => 'Position deleted successfully!'];
    }
}
...
// ✅ Count total rows
$sql_count = "SELECT COUNT(*) FROM positions $where";
...
// ✅ Fetch data
$sql = "SELECT * FROM positions $where ORDER BY id DESC LIMIT ? OFFSET ?";
...
<h2 class="mb-4">Positions Management</h2>
...
<!-- ✅ TABLE -->
<div class="card">
    <div class="card-header bg-secondary text-white">Position List</div>
    ...
    <div class="card-body table-responsive">
        <table class="table table-bordered table-hover align-middle">
            ...
            <tbody>
                <?php if ($positions): foreach ($positions as $row): ?>
                <tr>
