...
try {
    ...
    // ✅ Check for duplicate TD No + Tax Year in collections
    while ($checkRow = $result->fetch_assoc()) {
        $td_no_check = $checkRow['td_no'];
        $tax_year_check = $checkRow['tax_year'];

        $stmt_dup = $mysqli->prepare("SELECT COUNT(*) FROM collections WHERE td_no = ? AND tax_year = ?");
        $stmt_dup->bind_param('ss', $td_no_check, $tax_year_check);
        $stmt_dup->execute();
        ...
        if ($dup_count > 0) {
            ...
            echo json_encode([
                'success' => false,
                'message' => "Tax Declaration Number $td_no_check for year $tax_year_check is already paid."
            ]);
            exit;
        }
    }
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $stmt->bind_param(
            $types,
            $or_no, $previous_or_no, $previous_date_paid, $previous_year,
            $assessment_id, $tax_year, $rptsp_no, $td_no, $lot_no, $owner_name_row,
            $barangay, $location, $classification, $assessed_value, $basic_tax,
            $sef_tax, $tax_due, $adjustments, $discount, $penalty, $total_due,
            $payment_date, $status, $collecting_officer, $owner_col, $payor,
            $tax_year, $owner_id, $amount, $amount, $payment_method, $bank_name,
            $check_date, $check_amount, $check_number, $total_cash_amount
        );
