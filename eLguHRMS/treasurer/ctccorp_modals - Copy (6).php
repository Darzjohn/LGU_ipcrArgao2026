...
<script>
// ======== DUPLICATE CHECK & AUTO-SUGGEST CTC NUMBER ======== //

// Add modal
const addCtcInput = document.querySelector('#addCTCCorpModal [name="ctccorp_no"]');
if (addCtcInput) {
    addCtcInput.addEventListener('blur', e => {
        const val = e.target.value.trim();
        const year = document.querySelector('#addCTCCorpModal [name="year"]').value;
        if (!val) return;

        fetch(`ctccorp_list.php?check_ctccorp_no=${encodeURIComponent(val)}&year=${encodeURIComponent(year)}`)
            .then(res => res.json())
            .then(data => {
                if (data.duplicate) {
                    alert(`❌ Duplicate CTC Number!\nSuggested: ${data.suggested}`);
                    e.target.value = data.suggested;
                    e.target.focus();
                    computeCTC(document.querySelector('#addCTCCorpForm'));
                }
            });
    });
}
...
// Edit modals
document.querySelectorAll('.edit-ctccorp-no').forEach(input => {
    input.addEventListener('blur', e => {
        ...
        fetch(`ctccorp_list.php?check_ctccorp_no=${encodeURIComponent(val)}&year=${encodeURIComponent(year)}`)
            .then(res => res.json())
            .then(data => {
                if (data.duplicate && data.suggested != val) {
                    alert(`❌ Duplicate CTC Number!\nSuggested: ${data.suggested}`);
                    e.target.value = data.suggested;
                    e.target.focus();
                    computeCTC(form, true);
                }
            });
    });
});

// ======== FIXED AUTO COMPUTATION SCRIPT (accurate + stable timing) ======== //
function computeCTC(parent, isEdit = false) {
  ...
  const surcharge = getVal('surcharge');

  // ✅ Fix floating-point errors by rounding all intermediate results
  ...
  // ✅ Delay the conversion slightly so DOM updates finish first
  setTimeout(() => {
...
// ======== Consistent number-to-words conversion ======== //
function numberToWordsWithCents(value) {
  // ✅ Force exact rounding to 2 decimal places
