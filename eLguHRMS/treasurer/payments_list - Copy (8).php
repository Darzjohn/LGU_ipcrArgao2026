...
<div class="container-fluid mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="d-flex gap-2">
            ...
            <!-- Add this inside your .container-fluid above the table -->
...
$(function(){
    ...
    // Pass selected IDs & show summary + auto-fill total_cash_amount
    $('#proceedPaymentBtn').on('click', function(){
        var ids = $('.row-checkbox:checked').map(function(){ return $(this).val(); }).get();
        $('#selected_ids_input').val(ids.join(','));
        var $summaryContainer = $('#summaryContainer');
        var $tbody = $('#selectedSummaryTable tbody');
        var $grand = $('#grandTotalCell');
        $tbody.empty();

        if(ids.length === 0){
            $summaryContainer.hide();
            $tbody.append('<tr><td colspan="5" class="text-center text-muted">No records selected.</td></tr>');
            $grand.text('₱0.00'); return;
        }
        ...
        ids.forEach(function(id) {
            ...
            var td_no = row.find('td:nth-child(5)').text().trim();
            var year = row.find('td:nth-child(7)').text().trim();
            var discount = parseFloat(row.find('[data-field="discount"]').text().replace(/[₱,]/g,'').trim()) || 0;
            var penalty = parseFloat(row.find('[data-field="penalty"]').text().replace(/[₱,]/g,'').trim()) || 0;
            var total_due = parseFloat(row.find('[data-field="total_due"]').text().replace(/[₱,]/g,'').trim()) || 0;
            grandTotal += total_due;

            var rowClass = penalty > 0 ? 'table-danger' : '';
            var overdueBadge = penalty > 0
                ? `<span class="badge bg-danger ms-2" data-bs-toggle="tooltip" title="₱${penalty.toLocaleString(undefined,{minimumFractionDigits:2})} penalty">Overdue</span>`
                : '';
            $tbody.append(`
                <tr class="${rowClass}">
                    <td>${td_no}</td>
                    <td>${year} ${overdueBadge}</td>
                    <td>₱${discount.toLocaleString(undefined,{minimumFractionDigits:2})}</td>
                    <td>₱${penalty.toLocaleString(undefined,{minimumFractionDigits:2})}</td>
                    <td>₱${total_due.toLocaleString(undefined,{minimumFractionDigits:2})}</td>
                </tr>
            `);
        });

        $grand.text('₱'+grandTotal.toLocaleString(undefined,{minimumFractionDigits:2}));
    ...
    $(document).on('blur','[contenteditable="true"]', function(){
        ...
        if(['assessed_value','basic_tax','sef_tax','adjustments','discount','penalty','total_due'].includes(field)){
            value = value.replace(/[₱,]/g,'').trim(); if(value==='') value='0';
        }
...
$(function(){
    // Enable buttons on checkbox change (original + delete duplicates)
    function toggleButtons(){
        let hasSelection = $('.row-checkbox:checked').length > 0;
    ...
    // Delete Duplicate button click
    $('#deleteDuplicateBtn').on('click', function(){
        var ids = $('.row-checkbox:checked').map(function(){ return $(this).val(); }).get();
        if(ids.length === 0) return alert('Select at least one record.');

        if(!confirm('Are you sure you want to delete selected duplicate(s)? This cannot be undone.')) return;

        $.post('delete_duplicate.php', { ids: ids.join(',') }, function(resp){
            if(resp.success){
                alert('Selected duplicate(s) deleted successfully.');
                location.reload();
            } else {
                alert('Error: ' + (resp.message || 'Unable to delete.'));
            }
        }, 'json').fail(function(xhr){
            alert('Server error:\n' + xhr.responseText);
        });
