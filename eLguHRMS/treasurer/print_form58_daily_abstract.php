...
// Fetch Treasurer from signatories table
...
// Fetch Form58 records for the given date & collector
$stmt = $mysqli->prepare("
    SELECT id, or_no, date_paid, payor_name, amount_received
    FROM form58
...
<head>
  <meta charset="UTF-8">
  <title>Form 58 Daily Abstract</title>
  ...
  <style>
    ...
    .header-text { text-align: center; font-weight: bold; line-height: 1.2; margin-bottom: 25px; }
    .header-text img { width: 85px; height: 85px; object-fit: contain; margin-bottom: 8px; }
    .header-text h6 { font-size: 15px; margin: 2px 0; text-transform: uppercase; }
    .header-text h5 { font-size: 18px; text-decoration: underline; margin-top: 10px; color: #2b2b2b; }

    table { width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 13px; background: #fff; }
    th, td { border: 1px solid #000; padding: 7px 9px; vertical-align: middle; word-wrap: break-word; }
    th { background: #e9ecef; text-align: center; font-weight: bold; }
    ...
    th:nth-child(4), td:nth-child(4) { width: 40%; padding-left: 8px; }
    ...
    td.text-end { text-align: right; padding-right: 10px; }
...
<div class="container mt-4 mb-5 report-container">
  ...
  <div class="header-text">
    ...
    <h5>DAILY ABSTRACT OF FORM 58 COLLECTIONS PER COLLECTOR</h5>
  </div>
      ...
      <?php if ($result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td><?= date('n/j/Y', strtotime($row['date_paid'])) ?></td>
            <td><?= htmlspecialchars($row['payor_name']) ?></td>
            <td><?= '636-' . substr($row['id'], -1) ?></td>
            <td class="text-end">₱ <?= number_format($row['amount_received'], 2) ?></td>
          </tr>
          <?php $total += $row['amount_received']; ?>
        <?php endwhile; ?>
      <?php else: ?>
