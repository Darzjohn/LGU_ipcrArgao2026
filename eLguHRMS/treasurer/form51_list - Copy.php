...
if (isset($_POST['add_form51'])) {
    q(
        "INSERT INTO form51 (or_no, date_issued, payor_name, address, particulars, amount, cashier, created_by, created_at)
         VALUES (?,?,?,?,?,?,?,?,NOW())",
        [
            $_POST['or_no'], $_POST['date_issued'], $_POST['payor_name'], $_POST['address'],
            $_POST['particulars'], $_POST['amount'], $_SESSION['name'], $_SESSION['name']
        ]
    );
...
if (isset($_POST['edit_form51'])) {
    q(
        "UPDATE form51 SET or_no=?, date_issued=?, payor_name=?, address=?, particulars=?, amount=?, cashier=?, updated_by=?, updated_at=NOW() WHERE id=?",
        [
            $_POST['or_no'], $_POST['date_issued'], $_POST['payor_name'], $_POST['address'],
            $_POST['particulars'], $_POST['amount'], $_POST['cashier'], $_SESSION['name'], $_POST['form51_id']
        ]
    );
    echo "<script>alert('✏️ Form 51 updated successfully!'); location.href='form51_list.php';</script>";
    exit;
}
...
if (isset($_GET['delete'])) {
    ...
    echo "<script>alert('🗑️ Form 51 deleted successfully!'); location.href='form51_list.php';</script>";
    exit;
}
    ...
    <div class="card-body table-responsive">
      <table class="table table-bordered table-hover align-middle">
        <thead class="table-dark text-center">
          <tr>
            <th>ID</th><th>OR No</th><th>Payor Name</th><th>Address</th><th>Amount</th><th>Date Issued</th><th>Cashier</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
