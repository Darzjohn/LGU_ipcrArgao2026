      ...
      <?php if ($res && $res->num_rows > 0): ?>
        <?php while($row=$res->fetch_assoc()): ?>
          ...
          <tr>
            ...
            <td><?=strtolower($row['status'])=='paid'?'<span class="badge bg-success">Paid</span>':'<span class="badge bg-danger">Unpaid</span>'?></td>
          </tr>
        <?php endwhile; ?>
      <?php else: ?>
...
$(function(){
  $('#select_all').change(function(){
    $('input[name="selected_years[]"]').prop('checked',this.checked);
    togglePayButton();
  });
  $('input[name="selected_years[]"]').change(function(){
    if($('input[name="selected_years[]"]:checked').length==0) $('#select_all').prop('checked',false);
    else if($('input[name="selected_years[]"]:checked').length==$('[name="selected_years[]"]').length) $('#select_all').prop('checked',true);
    togglePayButton();
  });
