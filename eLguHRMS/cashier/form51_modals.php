<?php
// Suggested OR No - get last OR number for the current user
$current_user = $_SESSION['name'] ?? '';
$stmt = $mysqli->prepare("SELECT or_no FROM form51 WHERE created_by = ? ORDER BY id DESC LIMIT 1");
$stmt->bind_param('s', $current_user);
$stmt->execute();
$last_or = $stmt->get_result()->fetch_assoc()['or_no'] ?? '';
$suggest_form51_or = $last_or ? ((int)$last_or + 1) : 1;
?>
    ...
    <form method="POST" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row mb-2">
          <div class="col-md-4">
            <label>OR No</label>
            <input type="number" name="or_no" class="form-control" id="addOrNo" required>
          </div>
          <div class="col-md-4">
...
<!-- JS for suggested OR, dynamic table, payment -->
<script>
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateGrandTotal(){
    document.querySelectorAll('#form51ItemsTable, .editForm51ItemsTable').forEach(tbl => {
      ...
      const grandInput = form.querySelector('input[name="grand_total"]');
      ...
      if(mode === 'cash'){
        ...
        ['check_number','bank_name','check_date','check_amount'].forEach(id=>{
  ...
  // Initialize tables
  const addTable = document.getElementById('form51ItemsTable');
  if(addTable) setupTable(addTable);
...
document.addEventListener('DOMContentLoaded', function() {
  // Auto-suggest OR No
  const addOrInput = document.getElementById('addOrNo');
  if(addOrInput){
    addOrInput.value = "<?= $suggest_form51_or ?>";
  }
});
</script>
