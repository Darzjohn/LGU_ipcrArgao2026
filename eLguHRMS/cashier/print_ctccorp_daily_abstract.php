...
// Fetch CTC Corporation records for the given date & collector
$stmt = $mysqli->prepare("
    SELECT id, ctccorp_no, date_issued, company_fullname, total_due
    FROM ctc_corporation
...
<head>
  <meta charset="UTF-8">
  <title>Daily CTC Corporation Abstract</title>
  ...
  <style>
    ...
    th:nth-child(4), td:nth-child(4) { width: 37%; padding-left: 8px; }
...
<div class="container mt-4 mb-5 report-container">
  ...
  <div class="header-text">
    ...
    <h5>DAILY ABSTRACT OF CTC CORPORATION COLLECTION PER COLLECTOR</h5>
  </div>
    ...
    <tbody>
      <?php if ($result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            <td>CTC-CORP</td>
            <td><?= htmlspecialchars($row['ctccorp_no']) ?></td>
            <td><?= date('n/j/Y', strtotime($row['date_issued'])) ?></td>
            <td><?= htmlspecialchars($row['company_fullname']) ?></td>
            <td></td>
