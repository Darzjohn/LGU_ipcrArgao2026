...
foreach($properties as $pid=>$data){
    ...
    // Table header: added Tax Due After SEF column
    $html = '<table border="1" cellpadding="2" width="100%">
    <tr style="background-color:#f0f0f0;font-weight:bold;" align="center">
        <th>RPTSP</th><th>Year</th><th>Basic</th><th>SEF</th><th>Tax Due (Basic+SEF)</th><th>Adj</th><th>Disc</th><th>Pen</th><th>Total</th>
    </tr>';
    $totalAll=0;
    $today = new DateTime();
    ...
    foreach($data['bills'] as $r){
        ...
        // --- Tax after SEF ---
        $tax_after_sef = $basic + $sef;

        // --- Discount ---
        $discount = 0;
        $discountLabel = "";
        $discountPercent = 0;
        // 10% early payment for current year within Jan-Mar
        if ($tax_year == $curYear && $curMonth >= 1 && $curMonth <= 3) {
            $discountPercent = 10;
            $discount = 0.10 * $tax_after_sef;
            $discountLabel = $discountPercent . '%';
        }
        // if other discount rules apply (e.g., 20% next year Oct-Dec), add similarly:
        elseif ($tax_year == $curYear + 1 && $curMonth >= 10 && $curMonth <= 12) {
            $discountPercent = 20;
            $discount = 0.20 * $tax_after_sef;
            $discountLabel = $discountPercent . '%';
        }

        // --- Penalty ---
        // If discount applies, penalty = 0
        $penalty = 0;
        $penaltyLabel = "";
        $penaltyPercent = 0;
        ...
        if ($discountPercent == 0) {
            ...
            $rawPenaltyRate = 0.02 * $months_due; // 2% per month
            $appliedRate = min($rawPenaltyRate, 0.72); // cap at 72%
            $penaltyPercent = round($appliedRate * 100, 0);
            $penalty = $appliedRate * $tax_after_sef;
            if ($penaltyPercent > 0) $penaltyLabel = $penaltyPercent . '%';
        } else {
            // Explicitly zero penalty label when discount applied
            $penalty = 0;
            $penaltyLabel = '';
        }

        // --- Total ---
        $total = $tax_after_sef + $adj - $discount + $penalty;
        $totalAll += $total;

        // // --- Coloring for discount and penalty ---
        // // Discount: green when present
        // $discountHtml = $discount > 0
        //     ? '<span style="color:#0b6623;">' . number_format($discount,2) . ' ' . $discountLabel . '</span>'
        //     : '<span style="color:#666;">' . number_format(0,2) . '</span>';

        // // Penalty: red when present, muted when zero
        // $penaltyHtml = $penalty > 0
        //     ? '<span style="color:#c9302c;">' . number_format($penalty,2) . ' ' . $penaltyLabel . '</span>'
        //     : '<span style="color:#666;">' . number_format(0,2) . '</span>';

        // --- Prepare colored HTML for TCPDF ---
$discountHtml = number_format($discount, 2);
if ($discountPercent > 0) {
    $discountHtml .= ' (<span style="color:#0b6623;">'.$discountPercent.'%</span>)';
}

$penaltyHtml = number_format($penalty, 2);
if ($penaltyPercent > 0) {
    $penaltyHtml .= ' (<span style="color:#c9302c;">'.$penaltyPercent.'%</span>)';
}
        ...
        $html .= '<tr>
            ...
            <td align="right"><b>'.number_format($tax_after_sef,2).'</b></td>
            <td align="right">'.number_format($adj,2).'</td>
            <td align="right">'.$discountHtml.'</td>
            <td align="right">'.$penaltyHtml.'</td>
