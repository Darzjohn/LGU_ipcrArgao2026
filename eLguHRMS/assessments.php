...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_assessment'])) {
    ...
    if ($property_id > 0 && $tax_year > 0) {
        ...
        q("INSERT INTO assessments (property_id, tax_year, assessed_value, basic_tax_rate, basic_tax, sef_tax, adjustments, status)
           VALUES (?,?,?,?,?,?,?,?)",
           "iiddddss", [$property_id, $tax_year, $assessed_value, $basic_tax_rate, $basic_tax, $sef_tax, $adjustments, 'draft']);
...
<div class="modal fade" id="editModal" tabindex="-1">
  <div class="modal-dialog">
    <form method="post" class="modal-content">
      <div class="modal-header"><h5 class="modal-title">Edit Assessment</h5>
      ...
      <div class="modal-body row g-3">
        ...
        <div class="col-md-6">
          <label class="form-label">Tax Year</label>
          <input type="number" name="tax_year" id="edit-taxyear" class="form-control">
        </div>
        <div class="col-md-6">
          <label class="form-label">Assessed Value</label>
        ...
        <div class="col-md-6">
          <label class="form-label">Basic Tax Rate</label>
          <input type="number" step="0.01" name="basic_tax_rate" id="edit-rate" class="form-control">
        </div>
        <div class="col-md-6">
          <label class="form-label">Adjustments</label>
          <input type="number" step="0.01" name="adjustments" id="edit-adj" class="form-control">
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-primary" name="update_assessment">Save changes</button>
      </div>
    </form>
  </div>
</div>
...
document.getElementById('property-select').addEventListener('change', function(){
    ...
    document.getElementById('assessed-value-input').value = opt.getAttribute('data-assessed') || '';
    document.getElementById('tax-year-input').value = opt.getAttribute('data-effective') || '';
    document.getElementById('barangay-input').value = opt.getAttribute('data-barangay') || '';
    document.getElementById('location-input').value = opt.getAttribute('data-location') || '';
});
...
editModal.addEventListener('show.bs.modal', function (event) {
  ...
  document.getElementById('edit-taxyear').value = button.getAttribute('data-taxyear');
  document.getElementById('edit-value').value = button.getAttribute('data-value');
  document.getElementById('edit-rate').value = button.getAttribute('data-rate');
  document.getElementById('edit-adj').value = button.getAttribute('data-adj');
});
</script>
