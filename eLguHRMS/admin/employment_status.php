...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_status'])) {
    ...
    if ($name !== '') {
        $stmt = $mysqli->prepare("INSERT INTO employment_status (name) VALUES (?)");
        $stmt->bind_param("s", $name);
        $stmt->execute();
        $stmt->close();

        $messages[] = ['type' => 'success', 'text' => 'Employment Status added successfully!'];
    } else {
        $messages[] = ['type' => 'danger', 'text' => 'Status name is required!'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_status'])) {
    ...
    if ($id > 0 && $name !== '') {
        $stmt = $mysqli->prepare("UPDATE employment_status SET name=? WHERE id=?");
        $stmt->bind_param("si", $name, $id);
        $stmt->execute();
        $stmt->close();

        $messages[] = ['type' => 'success', 'text' => 'Employment Status updated successfully!'];
    } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($id > 0) {
        $stmt = $mysqli->prepare("DELETE FROM employment_status WHERE id=?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->close();

        $messages[] = ['type' => 'success', 'text' => 'Employment Status deleted successfully!'];
    }
}
...
// Count total
$sql_count = "SELECT COUNT(*) FROM employment_status $where";
...
// Fetch data
$sql = "SELECT * FROM employment_status $where ORDER BY id DESC LIMIT ? OFFSET ?";
...
<h2 class="mb-4">Employment Status Management</h2>
...
<div class="mb-3">
    <form method="get" class="row g-2">
        ...
        <div class="col-md-4">
            ...
            <a href="employment_status.php" class="btn btn-secondary">
...
<div class="mb-3 text-end">
    <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addStatusModal">
        <i class="bi bi-plus-circle"></i> Add Employment Status
    </button>
</div>

<!-- TABLE -->
<div class="card">
    <div class="card-header bg-secondary text-white">Employment Status List</div>
    ...
    <div class="card-body table-responsive">
        <table class="table table-bordered table-hover align-middle">
            ...
            <tbody>
                <?php if ($statuses): foreach ($statuses as $row): ?>
                <tr>
