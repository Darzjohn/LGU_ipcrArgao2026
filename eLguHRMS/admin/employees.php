...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // --- Fix appointment_date to allow NULL ---
    $appointment_date_raw = post('appointment_date');
    $appointment_date = ($appointment_date_raw && $appointment_date_raw !== '') ? $appointment_date_raw : null;
    ...
    if ($action === 'add') {
        ...
        $stmt = $mysqli->prepare("
            INSERT INTO employees (
                ...
                department_id, position_id, employment_status_id, appointment_date, 
        ...
        $stmt->bind_param(
            "sssssiiissssssssssssssssssssssssssssssssssss",
    ...
    if ($action === 'edit') {
        ...
        $id = (int)($_POST['employee_id'] ?? $_POST['id'] ?? 0);
...
if ($id <= 0) {
    die('Invalid employee ID.');
        ...
        if (!$photo_filename) {
            ...
            //                          ->fetch_assoc()['photo'];
            $stmtPhoto = $mysqli->prepare("SELECT photo FROM employees WHERE id=?");
$stmtPhoto->bind_param("i", $id);
$stmtPhoto->execute();
$resPhoto = $stmtPhoto->get_result()->fetch_assoc();
$photo_filename = $resPhoto['photo'] ?? null;
$stmtPhoto->close();
        ...
        $stmt->bind_param(
            "sssssiiissssssssssssssssssssssssssssssssssssi",
            $emp_idno, $photo_filename, $email, $dob, $blood_type,
            $department_id, $position_id, $employment_status_id, $appointment_date,
            $sss_no, $gsis_no, $tin_no, $pagibig_no, $phic_no,
            $emergency_contact_person, $emergency_contact_no,
                ...
                <?php if ($employees->num_rows > 0): ?>
                    <?php while ($e = $employees->fetch_assoc()): ?>
                        ...
                        <tr>
                            ...
                            <td><?= htmlspecialchars($e['appointment_date']) ?></td>
